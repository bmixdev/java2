...
import ru.mbelin.java3_hw6_logger_test.Task3;

import java.util.logging.Logger;

import static org.junit.Assert.assertTrue;

public class HW6Task3Test {
    private static final Logger log = Logger.getLogger(Task3.class.getName());

    @Test
    public void testTask2AnotherDigits() {
        assertTrue("В массиве есть другие цифры", Task3.process(new int[]{1, 1, 4, 1, 4, 4, 1, 1, 4}));
        log.info("Массив только из 1 и 4");
    }

    @Test(expected = RuntimeException.class)
    public void testTask2Only1And4() {
        Assert.assertFalse("В массиве только 1 и 4", Task3.process(new int[]{1, 4, 2}));
    ...
    public void testTask2LongWait() {
        ...
        assertTrue("Ошибка", Task3.process(data));
    }

    @Test
    public void testTask2contains1() {
        Assert.assertFalse("В массиве есть 1", Task3.process(new int[]{4,4,4,4,4}));
        log.info("Массив только из 4");
    }

    @Test
    public void testTask2contains4() {
        Assert.assertFalse("В массиве есть 4", Task3.process(new int[]{1,1,1,1,1}));
